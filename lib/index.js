...
const filterRequestCommand = `| grep url | head -n 1 | tr -s " " | cut -f4 -d " " | cut -f2 -d "=" | tr -d ">"`;
const allowedCodesArray = ["200", "201", "204", "400", "401", "403", "404", "500", "503"];
const sortBy = ['relevant', 'recent'];
...
const makeURL = (code) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    resolve(`${curlCommand} ${baseURL}${wordForURL}?sort=${sortByForURL} ${filterRequestCommand}`);
  });
};
